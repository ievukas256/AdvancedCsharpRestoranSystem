...
{
    public class RestoranReportRepository
    {
        public string CreateRestoranReport(int orderId)
        {
            using (StreamReader r = new StreamReader(@"C:\Users\sibai\Desktop\mokslai\Visual studio\AdvancedExamRestoran\DataFiles\OrdersData.json"))
            {
                ...
                var ordersList = JsonSerializer.Deserialize<List<RestoranReportItems>>(json);
                foreach (var items in ordersList)
                {
                    if (items.OrderId == orderId)
                    {
                        Console.WriteLine("BILL FOR RESTAURANT");
                        Console.WriteLine($"Table {items.TableId}, Order number {items.OrderId}, Date {items.Date}, Total Price {items.TotalPrice} eur ");
                    }
                }
        ...
        public string GetHtmlFromRestaurantBill(string file)
        {
            var json = File.ReadAllText(file);
            var list = JsonSerializer.Deserialize<List<RestoranReportItems>>(json);

            string html = "<table cellpadding='5' cellspacing='0' style='border: 1px solid #ccc; font-size: 12pt'>";
            html += "<h3>Bill for Restaurant:</h3>";
            html += "<tr>";
            ...
            File.WriteAllText(@"C:\Users\sibai\Desktop\mokslai\Visual studio\AdvancedExamRestoran\DataFiles\RestaurantBill.html", html);
            Console.WriteLine("Html file created.");
            var path = (@"C:\Users\sibai\Desktop\mokslai\Visual studio\AdvancedExamRestoran\DataFiles\RestaurantBill.html");
            return path;
        }
    }
}
